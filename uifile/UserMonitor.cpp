    ...
    bLoginout(false)
{
    ...
    {
        pWid->setFocusPolicy(Qt::NoFocus);
    }

    ui->widget_VideoDownloadWin->setFocusPolicy(Qt::NoFocus);
    ...
    m_pUpdateConnectTimer->setInterval(1000);
    ...
    m_pSearchTimer->setInterval(1000);
...
void UserMonitor::SetControlStyleSheet()
{
    if(iVideoFormat == VIDEO_1024_768)
    {
       ...
       // ui->widget_VideoDownloadWin->setStyleSheet("background:black;border:2px solid rgb(255,255,255);");
        ...
        ui->pushButtonSaveStatus->setStyleSheet("QPushButton{border-image: url(:/ctrl/Control/save_status.png)}");
    ...
    else
    {
       ...
       // ui->widget_VideoDownloadWin->setStyleSheet("background:black;border:2px solid rgb(255,255,255);");
        ...
        ui->pushButtonSaveStatus->setStyleSheet("QPushButton{border-image: url(:/ctrl/Control_800/save_status.png)}");
...
void UserMonitor::InitControl()
{
    ...
    if(g_iCarriageNum != MAX_CARRIAGE_NUM)
    {
        ...
        ui->pushButton_BackCar7->hide();
        ui->pushButton_BackCar8->hide();
    }
    ...
    ui->labelPlayBackCarChose->setText("<font color=white>回放车厢选择</b></font>");
    ui->labelPlayBackCarCtrl->setText("<font color=white>播放控制</b></font>");
    ...
    {
       ui->comboBoxCarriageSelect->addItem(C2S("车厢%1").arg(i+1));
    }
    ...
    headerText.clear();
    headerText<<"设备名"<<"设备IP"<<"硬盘容量"<<"硬盘使用量"<<"设备状态"<<" ";
 ...
 void UserMonitor::InitParams()
 {
    ...
    // 带检查功能，如果已有数据表则不创建
     ...
     if(bInitUser ==false)
     {
         QString sqlInsert = QString("insert into tb_power(User_name, User_operator, User_password) Values('%1', '%2', '%3');").arg("admin").arg(E_USER_INFO_SUPER_ADMIN).arg("123456");
...
void UserMonitor::GetAlarmList()
{
    ...
    for (int iNvrNo = 0; iNvrNo < m_pSdk->GetNvrNum(); iNvrNo++)
    {
        ...
        while(pWarnList != NULL)
        {
            ...
            strText = C2S("NVR%1").arg(iNvrNo+1);
            pItem = new QTableWidgetItem(strText);
        ...
        for (int iCamNo = 0; iCamNo < tNvrInfo.iIPCNum; iCamNo++)
        {
            ...
            while(pWarnList != NULL)
            {
                ...
                if(strText.isEmpty())
                {
                   strText = C2S("%1车厢%2号相机").arg(iCarriageNo).arg((int)tNvrInfo.atIpcInfo[iCamNo].cPos);
                }
                pItem = new QTableWidgetItem(strText);
                ui->tableWidgetAlarmList->setItem(iCurRow, 2, pItem);
...
void UserMonitor::SetCarrriageStatus()
{
    ...
    else
    {
        ...
        ui->pushButtonDeviceStatus->setStyleSheet("QPushButton{border-image: url(:/ctrl/Control_800/device_status_hot.png)}");
        int CarDriverAlarmX ,CarNVRX,CarAlarm1X,CarPantoX;
        int CarDriverAlarmY,CarNVRY,CarAlarm1Y,CarPantoY;
        if(g_iCarriageNum ==8)
        {
...
//退出
 void UserMonitor::on_pushButtonExit_clicked()
 {
     ...
     QMessageBox::StandardButton rb =QMessageBox::question(this, "视频监控系统", "是否退出该监控系统");
...
void UserMonitor::SearchFileTimerSlots()
{
    ...
    if(iFileCount != m_iSearchFileCnt)
    {
        iWaitCnt = 0;
        m_iSearchFileCnt = iFileCount;
        return;
    }
    else if(iFileCount > 0)
    {
        ...
        STATE_SetFileSearchState(E_FILE_IDLE);
        int iDataLen = 0;
        char *pBefore = NULL;
        char *pAfter = NULL;
        T_CMD_PACKET tPkt;
...
void UserMonitor::on_pushButtonStartQuery_clicked()
{
    ...
    if(sdt >= edt)
    {
        showWarn(this, C2S("提示"), C2S("搜索时间无效,请选择正确的开始/结束时间"));
        return;
    }
...
void UserMonitor::updateSearchFile()
{
    ...
    for(int i = 0; i < iCount; i++)
    {
        ...
        pItem = new QTableWidgetItem(C2S("车厢%1").arg(m_iSearchCarriageIndex));
        ui->tableWidgetFileList->setItem(i, 1, pItem);

        pItem = new QTableWidgetItem(C2S("相机%1").arg(tNode.iCam));
...
void UserMonitor::DownloadFileTimerSlots()
{
    ...
    if(iProgress < 100)
    {
        //ui->pushButtonDownload->setText(C2S("下载..."));
    }
    else
    {
        //ui->pushButtonDownload->setText(C2S("下载"));
        ...
        {
            showWarn(this, C2S(""), C2S("文件下载成功"));
        }
        else
        {
            showWarn(this, C2S(""), C2S("文件下载失败"));
        }
...
void UserMonitor::on_pushButtonDownload_clicked()
{
    ...
    {
        showWarn(this, C2S("警告"), C2S("未检测到U盘!"));
        return;
    }
    ...
    {
        showWarn(this, C2S("警告"), C2S("U盘不可写!"));
        return;
    }
    ...
    {
        showWarn(this, C2S("警告"), m_pSdk->GetErrorString());
    }
}
...
void UserMonitor::GetDeviceInfoOfCarriage(bool bUpdate)
{
    ...
    for (iNvrNo = 0; iNvrNo < m_pSdk->GetNvrNum(); iNvrNo++)
    {
        ...
        {
            strText = C2S("%1 G").arg(tNvrInfo.tState.sHdiskSize);
        }
        pItem = new QTableWidgetItem(strText);
        pItem->setTextAlignment(Qt::AlignCenter|Qt::AlignHCenter);
        ...
        {
            strText = C2S("%1 G").arg(tNvrInfo.tState.sHdiskUseSize);
        }
        pItem = new QTableWidgetItem(strText);
        pItem->setTextAlignment(Qt::AlignCenter|Qt::AlignHCenter);
        ...
        if (E_SERV_STATUS_CONNECT == NVR_GetConnectStatus(iNvrNo))
        {
            pItem = new QTableWidgetItem(C2S("在线"));
            pItem->setTextAlignment(Qt::AlignCenter|Qt::AlignHCenter);
        ...
        {
            pItem = new QTableWidgetItem(C2S("不在线"));
            pItem->setTextAlignment(Qt::AlignCenter|Qt::AlignHCenter);
            ui->tableWidgetSaveStatus->setItem(iCurRow,4,pItem);
        ...
        for (int iCamIndex = 0; iCamIndex < tNvrInfo.iIPCNum; iCamIndex++)
        {
            ...
            if(strText.isEmpty())
            {
               strText = C2S("%1车厢%2号相机").arg(iCarriageNo).arg((int)tNvrInfo.atIpcInfo[iCamIndex].cPos);
            }
            pItem = new QTableWidgetItem(strText);
            pItem->setTextAlignment(Qt::AlignCenter|Qt::AlignHCenter);
            ui->tableWidgetSaveStatus->setItem(iCurRow,0,pItem);
...
void UserMonitor::on_pushButtonSaveStatus_clicked()
{
     ...
     {
         ui->pushButtonDeviceStatus->setStyleSheet("QPushButton{border-image: url(:/ctrl/Control/device_status.png)}");
         ui->pushButtonSaveStatus->setStyleSheet("QPushButton{border-image: url(:/ctrl/Control/save_status_hot.png)}");
         ui->pushButtonAlarmList->setStyleSheet("QPushButton{border-image: url(:/ctrl/Control/alarm_list.png)}");
     }
     else
     {
         ui->pushButtonDeviceStatus->setStyleSheet("QPushButton{border-image: url(:/ctrl/Control_800/device_status.png)}");
         ui->pushButtonSaveStatus->setStyleSheet("QPushButton{border-image: url(:/ctrl/Control_800/save_status_hot.png)}");
         ui->pushButtonAlarmList->setStyleSheet("QPushButton{border-image: url(:/ctrl/Control_800/alarm_list.png)}");
     }
}
...
void UserMonitor::RegisterUserInfoSlots(QVariant variant,int nError)
{
    if(nError < 0)
    {
        if(nError == -1)
        {
            showWarn(this, C2S(""), C2S("输入参数无效"));
        }
        else if(nError == -2)
        {
            showWarn(this, C2S(""), C2S("密码不一致"));
        }
    ...
    {
        item = new QTableWidgetItem(C2S("管理员"));
    }
    else
    {
        item = new QTableWidgetItem(C2S("操作员"));
    }
...
void UserMonitor::loadUserInfo()
{
    ...
    while (query.next())
    {
        ...
        {
            item = new QTableWidgetItem(C2S("超级管理员"));
        }
...
void UserMonitor::on_pushButtonSetTime_clicked()
{
    if(m_nDateSetSelType == 0)
    {
        ...
        ui->pushButtonSetTime->setText(C2S("确定"));
...
unsigned int UserMonitor::UpdateFunc()
{
    ...
    if(sFilePath.isEmpty())
    {
        emit sglUpdateState(-1, C2S("未检测到U盘!"));
        return 0;
    }
   ...
   {
       emit sglUpdateState(-2, C2S("未检测到升级文件!"));
       return 0;
   }
#else
    ...
    for(int i=0; i<iFileNum; i++)
    {
        ...
        if (0 == iRet)
        {
            emit sglUpdateState(1, C2S("拷贝文件:%1!").arg(acValue));
#ifdef WIN32
            Sleep(1000);
#else
            usleep(100000);
    ...
    if(iUpdateFiles == iFileNum)
    {
        ...
        system(acBuf);
        emit sglUpdateState(1, C2S("拷贝文件成功，重启升级!"));
    }
    else
    {
        emit sglUpdateState(0, C2S("未拷贝文件!"));
    }
    return 0;
}
...
{
    if(QMessageBox::Yes == QMessageBox::question(this, C2S("提示"), C2S("是否确定要重启系统?")))
    {
        system("reboot");
        return;
    }
