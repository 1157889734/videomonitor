...
typedef struct _T_FTP_CONNECTION_INFO
{
    ...
    QMutex tQueueMutex;
} T_FTP_CONNECTION_INFO,*PT_FTP_CONNECTION_INFO;

PT_FTP_QUEUE FTP_CreateWorkQueue(QMutex *pMutex, int iQueueType)
{
...
static void  make_cwd_comm( char *dir_filename,char *buf_dir)
{
    ...
    strcpy(buf_dir,"CWD ");
	CHelpUint::UTF_8ToGB2312(&outlen, buf, tmpbuf, strlen(tmpbuf), 128);
    strcat(buf_dir,buf);
    strcat(buf_dir,"\r\n"); 
}
...
int FTP_SendCmd(T_FTP_CONNECTION_INFO *ftp, char *cmd)
{
    ...
    while (nLeft > 0)
    {
        int nWrite = 0;
#ifdef WIN32
        if ((nWrite = send (ftp->client_socket, cmd+write_pos, nLeft, 0)) <= 0)
#else
...
int FTP_CreateClientDataSocket(void)
{
    ...
    {   
        ...
        setsockopt(iSockFd, SOL_SOCKET, SO_REUSEADDR, (char*)&on, sizeof(on));
        return iSockFd;
    }
}
...
int FTP_CreateClientCmdSocket(void)
{
    ...
    {   
        setsockopt(iSockFd, SOL_SOCKET, SO_SNDTIMEO,(const char*) &nTimeout, sizeof(nTimeout));
        setsockopt(iSockFd, SOL_SOCKET, SO_RCVTIMEO,(const char*) &nTimeout, sizeof(nTimeout));
        setsockopt(iSockFd, SOL_SOCKET, SO_REUSEADDR,(const char*) &on, sizeof(int));
        return iSockFd;
    }
}
...
int FTP_ServerLogin(T_FTP_CONNECTION_INFO *ftp, long addr, int port, const char *uname,const char *pword)
{
    int err = 0; 
    int len = 0;
    char buf[128];
    ...
    server.sin_addr.s_addr = (addr);//inet_addr(addr);
    ...
    else
    {
        ...
        strcpy(tmpusername, "anonymous");
        strcpy(userpwd, "anon");
    }

	CHelpUint::UTF_8ToGB2312(&outlen, username, tmpusername, strlen(tmpusername), 128);
    //if ((strlen(username)==9)&&(strncasecmp(username, "anonymous", 9) == 0) && (strlen(userpwd) == 0))
    if ((outlen==9)&&(strnicmp(username, "anonymous", 9) == 0) && (strlen(userpwd) == 0))
...
int FTP_GetFileSize(T_FTP_CONNECTION_INFO *ftp, char *filepath, unsigned long long *filesize)
{
	...
	int outlen = 0;
	outlen = 0;
    CHelpUint::UTF_8ToGB2312(&outlen, sendfilepath, filepath, strlen(filepath), 256);
    sprintf(buf, "SIZE %s\r\n", sendfilepath);
    *filesize = 0;
    err = FTP_SendCmd(ftp, buf);
    if (err < 0)
...
int FTP_CheckdirAndMakedir(T_FTP_CONNECTION_INFO *ftp,  char *dname)
{
    ...
    {   
        ...
        strcpy(send_buf,"CWD ");
		outlen = 0;
        CHelpUint::UTF_8ToGB2312(&outlen, sendcheck_buf, check_buf, strlen(check_buf), 256);
        strcat(send_buf,sendcheck_buf);
        strcat(send_buf,"\r\n");
#ifdef WIN32
         if(send (ftp->client_socket, send_buf, strlen(send_buf), 0) <0)  //发送CWD命令目录
#else
        ...
        if((strstr(recv_buf,"550")!=NULL)||((strstr(recv_buf,"450")!=NULL)))
        {
			...
			outlen = 0;
       		CHelpUint::UTF_8ToGB2312(&outlen, gb2312send_buf, send_buf, strlen(send_buf), 100);
            strcat(str_mkd_2,gb2312send_buf);
            strcat(str_mkd_2,"\r\n");
#ifdef WIN32
               if(send (ftp->client_socket, str_mkd_2, strlen(str_mkd_2), 0) <0)  //发送CWD命令目录
#else
        ...
        {
#ifdef WIN32
            send(ftp->client_socket, str, strlen(str), 0);
#else
...
#ifdef WIN32
DWORD WINAPI FTP_DownloadDataRecvThread(void *param)
#else
...

{
    ...
    while (1 == ptFtpConnectionInfo->threadRunFlag)
    {
	    ...
	    if (iRet > 0)
	    {
		    ...
		    memset(buf,0,sizeof(buf));
			outlen = 0;
		    CHelpUint::UTF_8ToGB2312(&outlen, sendfilename, tFtpPkt.acSrcFileName, strlen(tFtpPkt.acSrcFileName), 256);
		    sprintf(buf, "RETR %s\r\n", sendfilename);
		    if ((iRet = FTP_SendCmd(ptFtpConnectionInfo, buf)) < 0)
		    {
...
#ifdef WIN32
DWORD WINAPI FTP_UploadDataSendThread(void *param)
#else
         ...
         
{
	...
	while (1 == ptFtpConnectionInfo->threadRunFlag && llFileSize < ptFtpConnectionInfo->uploadLocalFileSize)
    {
	    ...
	    if (iRet > 0)
	    {
		   ...
		   CHelpUint::UTF_8ToGB2312(&iOutlen, acSendFilePath, tFtpPkt.acDstFileName, strlen(tFtpPkt.acDstFileName), 256);
...
PFTP_HANDLE FTP_CreateConnect(const char *pcIpAddr, int iPort, PF_FTP_PROC_CALLBACK pFtpProcFunc, const char *pUserName,const char *pPassword)
{
	int iRet = 0;
    ...
    iRet = FTP_ServerLogin(ptFtpConnectionInfo, (inet_addr(pcIpAddr)), iPort, pUserName, pPassword);   //匿名方式登录服务器
	if (iRet < 0)
	{
		free(ptFtpConnectionInfo);
    	ptFtpConnectionInfo = NULL;
...
int FTP_AddDownLoadFile(PFTP_HANDLE pFtpHandle, char *pcSrcFileName,const char *pcDstFileName)
{
...
int FTP_FileDownLoad(PFTP_HANDLE pFtpHandle)
{
    ...
    iRet = (int)CreateThread( NULL, 0,(LPTHREAD_START_ROUTINE)FTP_DownloadDataRecvThread, (void *)ptFtpConnectionInfo,0,NULL);
...
#ifdef WIN32
	iRet = (int)CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)FTP_UploadDataSendThread, (void *)ptFtpConnectionInfo,0,NULL);
#else
	iRet = pthread_create(&ptPmsgConnInfo->ThreadHandle, NULL, FTP_UploadDataSendThread, (void *)ptFtpConnectionInfo);
	if (iRet < 0)
	{
		free(ptPmsgConnInfo);
		ptPmsgConnInfo = NULL;
