...
#endif
{
    ...
    while(1)
    {
		...
		//TRACE("seq %d len %d, type %d\n", ptRtpHeader->seq, iRecvLen, cCurPayloadType);

		//if (strstr(ptRtspConn->acUri, "72"))
		/*{

			char str[100];
			sprintf(str,"seq %d len %d, type %d\n", ptRtpHeader->seq, iRecvLen, cCurPayloadType);
...
int RtpParseH264Data(PT_RTSP_CONN ptRtspConn, char *pcFrameBuf, int iFrameLen, char *pcRtpData, int iDataLen, int iFrameLostFlag, int *piReqIFrameFlag, unsigned int ts)
{
	...
	while (iLeaveLen > 0)
	{
		...
		//TRACE("rtplen %d, type %d\n", iDataLen, cPacketType);
		...
		if (cRtpMFlag)
		{
			...
			{
				ptRtspConn->pfRtpSetDataCB(STREAM_TYPE_VIDEO, E_STREAM_TYPE_H264, pcFrameBuf, iFrameLen, ts, ptRtspConn->pRtpCbArg);
				//TRACE("a frame\n");
			}
