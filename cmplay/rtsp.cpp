...
T_SDP_INFO *RtspParseSdp(char *pcBuf, int iLen)
{
	...
	if (ptSdpInfo == NULL)
	{
		RTSP_DEBUG ("Cannot Allocate Memory to Request Message's Pointer\n");
		return NULL;
	}
	...
	for (;;)
	{
		...
		if ((indexCnt = FindCharInBuffer (buffPtr, '=', '\r', buffLen)))
		{
			...
			{
				RTSP_DEBUG("Memory Allocation Failed \n");
				break;
			}
...
T_RTSP_RESPONSE_MSG *RtspParseResponseMsg(char *pcBuf, int iLen)
{
    ...
    if ((indexCnt = FindCharInBuffer (buffPtr, ' ', '\r', buffLen)))
    {
        ...
        {
            RTSP_DEBUG ("Cannot Allocate Memory for tempArray Pointer\n");
            free(ptResponseMsg);
            ptResponseMsg = NULL;
            return NULL;
        }
    ...
    if ((indexCnt = FindCharInBuffer (buffPtr, ' ', '\r', buffLen)))
    {
        ...
        if(tempArray != NULL)
        {
            ...
            RTSP_DEBUG("Requested Status code = %d\n", ptResponseMsg->ResStatusCode);
            free(tempArray);
            tempArray = NULL;
...
int GetSpsPpsData(PT_RTSP_CONN ptRtspConn, T_SDP_INFO *ptSdpInfo)
{
    ...
    DebugPrint(DEBUG_RTSP_9, "[%s] %d\n", __FUNCTION__, ptSdpInfo->ResSdpHeaderCount);
	for (indexCnt = 0; indexCnt < ptSdpInfo->ResSdpHeaderCount; indexCnt++)
	{
		DebugPrint(DEBUG_RTSP_9, "%s\n", ptSdpInfo->ResMsgSdpHeaders[indexCnt].pHeaderValue);
		//printf("")ptSdpInfo->ResMsgSdpHeaders[indexCnt].pHeaderName, ptSdpInfo->ResMsgSdpHeaders[indexCnt].pHeaderValue;
		if( !strncmp(ptSdpInfo->ResMsgSdpHeaders[indexCnt].pHeaderValue, mtag,strlen(mtag)))
		{
...
INT32 AddRequestHeader (T_RTSP_REQUEST_MSG *ptRequestMsg, char *HeaderName, char *HeaderValue)
{
    ...
    {
        RTSP_DEBUG("Invalid header count of response message\n");
        return RET_ERROR;
    }
    ...
    {
        RTSP_DEBUG("Can not allocate memory for response header name\n");
        return RET_ERROR;
    }
    ...
    {
        RTSP_DEBUG("Can not allocate memory for response header value\n");
...
{
    BYTE    *pcRequestBuf = NULL;       /* RTSP request buffer pointer           */
    ...
    pcRequestBuf = (BYTE *) malloc (requestSize);

    if( pcRequestBuf == NULL )
    {
        RTSP_DEBUG("Can not allocate memory for response string\n");
        return RET_ERROR;
    }
	...
	if (ptRtspConn->pMute)
	{
        ptRtspConn->pMute->Lock();
	}
	if(send (ptRtspConn->iRtspSocket, (const char*)pcRequestBuf, requestSize , 0) == -1 )
	{
		if (ptRtspConn->pMute)
		{
			ptRtspConn->pMute->Unlock();
		}
	    RTSP_DEBUG("RtspSendRequest error\n");
	    free(pcRequestBuf);
	    pcRequestBuf = NULL;
...
#endif
{
	...
	RTSP_DEBUG("[RtspRecvThread] enter\n");
	...
	if (TCP == ptRtspConn->iRtpTransportProtocol)
	{
		...
		{
			DebugPrint(DEBUG_RTSP_9,"[RtspRecvThread] malloc pcVideoFrame Size Fail");
		...
		{
			DebugPrint(DEBUG_RTSP_9,"[RtspRecvThread] malloc pcAudioFrame Size Fail");
			goto RTSP_RECV_ERROR;
		}
		...
		if (NULL == pcLeaveBuf)
		{
			DebugPrint(DEBUG_RTSP_9,"[RtspRecvThread] malloc pcLeaveBuf Size Fail");
			goto RTSP_RECV_ERROR;
		}
    ...
    while (ptRtspConn->iRtspThreadRunFlag)
    {
		...
		{
			DebugPrint(DEBUG_RTSP_9,"preleavelen %d", iPreLeaveLen);
			iPreLeaveLen = 0;
		}
        ...
        {
            // procsee error
			DebugPrint(DEBUG_RTSP_9,"[RtspRecvThread] iRecvLen = %d iErrorCode = %d ",iRecvLen,GetLastError());
			DebugPrint(DEBUG_RTSP_9,"[RtspRecvThread] iPreLeaveLen = %d ilen = %d ",iPreLeaveLen,sizeof(acRecvBuf) - iPreLeaveLen - 1);
            goto RTSP_RECV_ERROR;
			...
			while (iLeaveLen > 0)
			{
				...
				if((pcData[0] & 0xFF) == '$')          
				{
					...
					else
					{
						...
						//	TRACE("iUdpSRTime %d, %d, cv %d\n", iUdpSRTime, ssrc, rtcp_recv->common.version);
				...
				else
				{
					...
					if (0 == iErrorFlag)
					{
						...
						DebugPrint(DEBUG_RTSP_9, "tmplen %d, leavelen %d, prelen %d,  prev %x, acTmp:%s", iTmpLen, iLeaveLen, iPreLeaveLen, cPrevStr, acTmp);
						...
						if ((0 == iLeaveLen) && ('\r' != cPrevStr) && ('\n' != *ptr))
						{
							...
							iPreLeaveLen = 0;
							iErrorFlag = 1;
							DebugPrint(DEBUG_RTSP_9, "rtsp response error\n");
						}
					}
					else
					{
						while (iLeaveLen > 0)
						{
							...
							if (('$' == cPrevStr) && ucChId < 4)
							{
								...
								iOffset -= 2;
								iErrorFlag = 0;
								DebugPrint(DEBUG_RTSP_9, "rtsp response error restore, %x, %x\n", cPrevStr, ucChId);
								break;
							}
							cPrevStr = *ptr;
							ptr++;
...
RTSP_RECV_ERROR:
	...
	RTSP_DEBUG("[RtspRecvThread] exit\n");
	DebugPrint(DEBUG_RTSP_9,"[RtspRecvThread] exit iRtspThreadRunFlag = %d",ptRtspConn->iRtspThreadRunFlag);
	if (pcVideoFrame)
	{
		free(pcVideoFrame);
		pcVideoFrame = NULL;
...
int ParseHostFromUri(const char *pcUri, char *pcHost, INT32 *piServPost)
{
    char *pcBuf = NULL;
    int iLen = 0;
    int indexCnt = 0;
...
int RtspConnect(PT_RTSP_CONN ptRtspConn, const char *pcUri)
{
    ...
    RTSP_DEBUG("Host %s, Port %d\n", ptRtspConn->acServHost, ptRtspConn->iRtspPort);
    ...
    if (ptRtspConn->iRtspSocket < 0)
    {		
        DebugPrint(DEBUG_RTSP_9, "Create rtsp socket error\n");
        return -1;
    }
    ...
    {
        RTSP_DEBUG("Set SO_SNDTIMEO error\n");
        RTSP_Close(ptRtspConn->iRtspSocket);
        ptRtspConn->iRtspSocket = -1;
        return -4;
    }
...
int RtspRequestOptions(PT_RTSP_CONN ptRtspConn)
{
    ...
    ptRtspConn->tRequestMsg.pReqMsgURI = ptRtspConn->acUri;
	  ...
	  if (iRet < 0)
	  {
	      RTSP_DEBUG("Rtsp send request error\n");
	      return -1;
	  }
    ...
    {
        RTSP_DEBUG("Rtsp get response msg error\n");
		if (ptRtspBuf)
		{
...
int RtspRequestDescribe(PT_RTSP_CONN ptRtspConn)
{
    ...
    DebugPrint(DEBUG_RTSP_9, "sdplen %s\n", pcHeaderValue);
