...
#define AUTHORIZATE_TYPE_DIGEST        1   // 摘要认证
#define AUTHORIZATE_TYPE_BASIC         2   // 基本认证
...
typedef struct _T_RTSP_CONN
{
	...
	int iAuthorizeFlag;      // 是否要认证
	char acUser[64];         // 用户名
	char acPasswd[64];       // 密码
	char acAuthRealm[32];    // 认证时服务器返回
	char acAuthNonce[64];    // 认证时服务器返回
	char acNewUri[256];      // 重构setup后的uri
    char acSession[64];     
	char acUri[256];
