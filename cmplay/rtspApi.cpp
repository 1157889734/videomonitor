...
INT32 CreateUdpSocket(INT32 iUdpPort)
{
    ...
    if ( setsockopt(iSocket, SOL_SOCKET, SO_REUSEADDR, \
                    (const char *)&iReuseFlag, sizeof(INT32)) == SOCKET_ERROR )
    {
            RTSP_DEBUG("Failed to set Reuseaddr\n");
    }
...
RTSP_HANDLE RTSP_Login(const char *pUrl, char *pcUser, char *pcPasswd)
{
   ...
   // T_RTSP_REQUEST_MSG tRequestMsg = {0};
    ...
    if (NULL == ptRtspConn)
    {
        RTSP_DEBUG("can't malloc memery");
        return 0;	
    }
	...
	ptRtspConn->pMute = new CMutexLock;
...
int RTSP_Logout(RTSP_HANDLE RHandle)
{
	...
	//QUEUE_DestroyWorkQueue(ptRtspConn->ptWorkQueue);
	if (ptRtspConn->pMute)
	{
		delete ptRtspConn->pMute;
		ptRtspConn->pMute = NULL;
	}
    free(ptRtspConn);
    ptRtspConn = NULL;

    RTSP_DEBUG("rtsp logout exit\n");
...
int RTSP_OpenStream(RTSP_HANDLE RHandle, int iCh, int iRtpTransportProtocol, void *pfRtpSetDataCB, void *pUserData)
{
    ...
    if (TCP == iRtpTransportProtocol)
    {
        ...
        {
            RTSP_DEBUG("Set SO_RCVTIMEO error\n");
            RTSP_Close(ptRtspConn->iRtspSocket);
            ptRtspConn->iRtspSocket = -1;
            return -4;
        }	
...
INT32 RTSP_CloseStream(RTSP_HANDLE RHandle, int iCh)
{
	PT_RTSP_CONN ptRtspConn = (PT_RTSP_CONN)RHandle;
	char acCseq[12];
	int iRet = 0;
...
INT32 RTSP_PlayControl(RTSP_HANDLE RHandle, int iCmd, double dValue)
{
    PT_RTSP_CONN ptRtspConn = (PT_RTSP_CONN)RHandle;
    int iRet = 0;
    char acBuf[512];
...
INT32 RTSP_GetServIpAddr(RTSP_HANDLE RHandle, char *pcIpAddr, int iLen)
{
    PT_RTSP_CONN ptRtspConn = (PT_RTSP_CONN)RHandle;
