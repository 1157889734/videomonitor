...
typedef IDirect3D9* WINAPI pDirect3DCreate9Ex(UINT SDKVersion,void **);;
...
void dxva2_uninit(AVCodecContext *s)
{
    ...
    if (ctx->d3d9device)
        IDirect3DDevice9Ex_Release(ctx->d3d9device);

    if (ctx->d3d9)
        IDirect3D9Ex_Release(ctx->d3d9);
...
static int dxva2_get_buffer(AVCodecContext *s, AVFrame *frame, int flags)
{
	...
	if(s->width != ist->initWidth || s->height != ist->initHeight)
	{
		ReDrawaDxva(s,s->width,s->height);
		ist->initWidth = s->width  ;
		ist->initHeight = s->height;
	}
...
static int dxva2_retrieve_data(AVCodecContext *s, AVFrame *frame)
{
    ...
    InputStream        *ist = (InputStream *)s->opaque;
    DXVA2Context       *ctx = (DXVA2Context *)ist->hwaccel_ctx;
	EnterCriticalSection(&ist->cs);
	ctx->d3d9device->Clear(0, NULL, D3DCLEAR_TARGET|D3DCLEAR_ZBUFFER|D3DCLEAR_STENCIL, D3DCOLOR_XRGB(0, 0, 255), 1.0f, 0);
...
static int dxva2_alloc(AVCodecContext *s, HWND hwnd)
{
    ...
    DXVA2Context *ctx;
    pDirect3DCreate9Ex      *createD3D = NULL;
    ...
    createD3D = (pDirect3DCreate9Ex *)GetProcAddress(ctx->d3dlib, "Direct3DCreate9Ex");
    if (!createD3D) {
    ...
    createD3D(D3D_SDK_VERSION,(void **)(&ctx->d3d9));
    if (!ctx->d3d9) {
	...
	if (SUCCEEDED(hr))
	{
		...
		{
			BehaviorFlags = D3DCREATE_HARDWARE_VERTEXPROCESSING | D3DCREATE_MULTITHREADED | D3DCREATE_FPU_PRESERVE|D3DCREATE_SCREENSAVER;
		}
		else
		{
			BehaviorFlags = D3DCREATE_SOFTWARE_VERTEXPROCESSING | D3DCREATE_MULTITHREADED | D3DCREATE_FPU_PRESERVE|D3DCREATE_SCREENSAVER;
		}
	}
	...
	hr = IDirect3D9Ex_CreateDeviceEx(ctx->d3d9, adapter, D3DDEVTYPE_HAL, hwnd, BehaviorFlags, &d3dpp,NULL, &ctx->d3d9device);
    if (FAILED(hr)) {
...
int dxva2_init(AVCodecContext *s, HWND hwnd)
{
    ...
    DXVA2Context *ctx;
    int ret;
	InitializeCriticalSection(&ist->cs);
...
int	ReDrawaDxva(AVCodecContext *s,int iWidth,int iHeight)
{
	if(NULL == s)
	{
		return -1;
