...
static void transform64Bytes(unsigned char const block[64])
{
	...
	for (k = 0; k < 16; ++k) x[k] = 0;
}


static void addData(unsigned char const* inputData, unsigned inputDataSize) {
...
static void zeroize() {
	...
	fBitCount = 0;
	for (i = 0; i < 64; ++i) fWorkingBuffer[i] = 0;
}

static void finalize(unsigned char* outputDigestInBytes) {
  // Unpack our bit count:
  unsigned char bitCountInBytes[8];

  u_int64_t byteCount = 0;
  unsigned bufferBytesInUse = 0;
  unsigned numPaddingBytes = 0;
  ...
  byteCount = fBitCount>>3;
  bufferBytesInUse = (unsigned)(byteCount&0x3F);
  numPaddingBytes
...
static void end(char* outputDigest) 
{
...
char* our_MD5Data(unsigned char const* data, unsigned dataSize, char outputDigest[DIGEST_SIZE_AS_STRING]) 
{
	start();
...
unsigned char* our_MD5DataRaw(unsigned char const* data, unsigned dataSize, unsigned char outputDigest[DIGEST_SIZE_IN_BYTES]) 
{
	start();

	addData(data, dataSize);

	finalize(outputDigest);
